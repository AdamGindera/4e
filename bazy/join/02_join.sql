-- − Utwórz bazę danych o nazwie szkolenia, z zestawem polskich znaków (np. utf8_polish_ci).  Z rozpakowanego archiwum zaimportuj tabele z pliku szkolimy.sql do utworzonej bazy
...
SELECT nazwa, COUNT(*) AS 'Zapisanych'
FROM Kursy INNER JOIN kursy_uczestnicy ON kursy.kod=kursy_uczestnicy.kod_kursu
GROUP BY nazwa;
...
SELECT cecha, COUNT(*) AS liczba_abonamentow
FROM cechy INNER JOIN szczegolyabonamentu ON cechy.id=szczegolyabonamentu.cechy_id
GROUP BY cecha;
